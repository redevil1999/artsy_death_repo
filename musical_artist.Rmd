...
ggplot( data = musical_artists_1800s) +
  geom_point( mapping = aes(x= as.numeric(birthYear), y = age)) + xlab("Birth Year") + ylab("age of death") +  ggsave('MusicalArtistDeath.jpeg')
...
musical_artist_group_age <- musical_artist_numeric %>%
  ...
  mutate(cuts = cut(numeric_birthYear, breaks=c(1814, 1860, 1905, 1950), labels=FALSE)) %>%
  mutate(cuts = ifelse(cuts == 1, '1815-1860', cuts)) %>%
  mutate(cuts = ifelse(cuts == 2, '1861-1905', ifelse(cuts == 3, '1906-1950', cuts)))%>%
  group_by(cuts)
